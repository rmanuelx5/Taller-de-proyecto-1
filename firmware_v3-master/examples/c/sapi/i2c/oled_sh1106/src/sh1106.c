...
static void SH1106_cmd(uint8_t cmd) {
	...
	buf[0] = 0x00;
	buf[1] = cmd;
	i2cWrite(SH1106_I2C_PORT, SH1106_I2C_ADDR, buf, 2, TRUE);
}
...
static void SH1106_cmd_double(uint8_t cmd1, uint8_t cmd2) {
	...
	buf[1] = cmd1;
	buf[2] = cmd2;
	i2cWrite(SH1106_I2C_PORT, SH1106_I2C_ADDR, buf, 3, TRUE);
}
...
void SH1106_Init(void) {
	i2cInit(SH1106_I2C_PORT, SH1106_I2C_RATE);
	delay(100);
...
void SH1106_Flush(void) {
	...
	for (uint32_t page = 0; page < bits_h; ++page) {
		...
		i2cWrite(SH1106_I2C_PORT, SH1106_I2C_ADDR, buf, 129, TRUE);
	}
}
